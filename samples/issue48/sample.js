...
import { default as asyncFunctionDefault, asyncFunction,  __ModuleAPI__ as AsyncFunctionModuleAPI  } from './src/AsyncFunction.js';
...
describe('Test for issue 48', function() {
  it('should be able to rewire default async function', function() {
    return asyncFunctionDefault().then(response => {
      ...
      AsyncFunctionModuleAPI.__set__('promiseFunction', function() {
        return Promise.resolve(3);
      });

      return asyncFunctionDefault().then(response => {
        expect(response).to.equal(3);
        AsyncFunctionModuleAPI.__ResetDependency__('promiseFunction');
      });
    });
  });
